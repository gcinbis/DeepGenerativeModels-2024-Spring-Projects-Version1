...
from codes.utils import standardize_batch
...
class Train:
    def __init__(self, config):
        ...
        if(self.verbose):
            ...
            self.print_network(self.style_transformer, 'StyleTransformer4')
    ...
    def train(self):
        ...
        coco_dataloader = DataLoader(coco_train_dataset_object,
                                     batch_size=self.batch_size_content, num_workers=self.num_workers, 
                                     pin_memory=self.pin_memory, drop_last=True, sampler=InfiniteSampler(coco_train_dataset_object))

        wikiart_train_dataset_object = wikiart_dataset(self.project_root, self.wikiart_dataset_path)
        wikiart_dataloader = DataLoader(wikiart_train_dataset_object, batch_size=self.batch_size_style, num_workers=self.num_workers, 
                                        pin_memory=self.pin_memory, drop_last=True, sampler=InfiniteSampler(wikiart_train_dataset_object))
        ...
        # Create omega style_transformer and decoder objects for meta training, and load the parameters
        if not self.freeze_encoder:
        ...
        for iteration in tqdm(range(1, self.max_iterations + 1)):
            ...
            if (self.batch_size_content == self.batch_size_style):
                style_image_batch = style_image
            elif (self.batch_size_content % self.batch_size_style) == 0:
                ...
                style_image_batch = torch.cat((style_image.repeat((self.batch_size_content // self.batch_size_style), 1, 1, 1),
                                              ...
                                              dim=0)
            style_image_batch = standardize_batch(style_image_batch)
            ...
            for inner_loop_index in range(1, self.num_inner_updates+1):
                ...
                content_images = standardize_batch(content_images)
                ...
                decoded_output = standardize_batch(decoded_output)
            ...
            # Update theta (actual) parameters with omega (fast) parameters
            ...
            # Save model periodically and log images to wandb
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_save_path', type=str, default="experiments/weights",)

    # Loss model path
    parser.add_argument('--loss_model_path', type=str, default="models/vgg_19_last_layer_is_relu_5_1_output.pt",
    ...
    parser.add_argument('--encoder_model_path', type=str, default='models/swin_B_first_2_stages.pt', help='Path where the Swin model is saved or should be saved.')
    ...
    parser.add_argument('--save_every', type=int, default=25, help='Save the model every n iterations')
    # Dataset paths
    ...
    # verbose
    parser.add_argument('--verbose', type=bool, default=False, help='Print the model informations and loss values at each loss calculation.')
