...
from util.dataset import raster_to_json 
import math
import random
import torch as th
...
import copy
def temp_wrapper(path):
    try:
        raster_to_json.raster_to_json(path, False)
        sys.stdout.flush()
...
def prepare_dataset(root="dataset/"):
    path_to_images = os.path.join(root, "floorplan_dataset")
    image_names = [f"dataset/floorplan_dataset/{p}" for p in os.listdir(path_to_images)]
    max_processes = multiprocessing.cpu_count()
    pool = multiprocessing.Pool(processes=max_processes) 
    for _ in tqdm.tqdm(pool.imap_unordered(temp_wrapper, image_names), total=len(image_names)):
        pass
...
def load_rplanhg_data(
    ...
    dataset_files = os.listdir(root_folder)
    if set_name == "eval":
        num_points = 10
    dataset_obj = RPlanhgDataset(analog_bit = analog_bit, target_set=  target_set, base_dir=root_folder, set_name=set_name , lines=dataset_files[20000:20000+num_points] if set_name == "eval" else dataset_files[:num_points])
    loader = DataLoader(
            dataset_obj, batch_size=batch_size, shuffle=True, num_workers=2, drop_last=False
    )
    while True:
        yield from loader
