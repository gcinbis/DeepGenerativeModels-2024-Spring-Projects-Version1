...
class RPlanEncoder(nn.Module):
    ...
    def forward(self, x, door_mask, self_mask, gen_mask):
        ...
        return x
class RPlanTransformer(nn.Module):
    def __init__(
            self,
        in_channels,
        condition_channels,
        ...
        self.transformer_layers = nn.ModuleList([RPlanEncoder(self.model_channels, 4, 0.1, self.activation) for x in range(self.num_layers)])
        # self.transformer_layers = nn.ModuleList([nn.TransformerRPlanEncoder(self.model_channels, 4, self.model_channels*2, 0.1, self.activation, batch_first=True) for x in range(self.num_layers)])
        ...
        if not self.analog_bit:
            ...
            self.output_linear_bin2 = RPlanEncoder(self.model_channels, 1, 0.1, self.activation)
            self.output_linear_bin3 = RPlanEncoder(self.model_channels, 1, 0.1, self.activation)
