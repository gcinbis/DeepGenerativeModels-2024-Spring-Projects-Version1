...
class TimestepEmbedding(nn.Module):
    def __init__(self, dim, max_period=10000):
        ...
        self.register_buffer('padding', torch.tensor(dim % 2))
...
class HouseDiffEncoder(nn.Module):
    def __init__(self, dim, num_heads, dropout=0.1):
        ...
        mha_kwargs = dict(embed_dim=dim, num_heads=num_heads, dropout=dropout, batch_first=True)
        ...
        self.attention = lambda attention, x, mask: self.dropout(attention(x, x, x, attn_mask=mask.repeat(num_heads, 1, 1).float(), need_weights=False)[0])
...
class HouseDiffTransformer(nn.Module):
    def __init__(
        ...
        def _binary_mlp_forward(x, self_mask, door_mask, gen_mask):
            for layer in self.binary_mlp:
                if isinstance(layer, HouseDiffEncoder):
                    x = layer(x, self_mask, door_mask, gen_mask)
                else:
                    x = layer(x)
            return x
        
        self.binary_mlp.forward = _binary_mlp_forward
    ...
    def compute_binary_output(self, x, output_decimal, xtalpha, epsalpha, conditions, condition_embeddings):
        """
        ...
        Args:
            ...
            conditions (dict): Conditions for the model.
        ...
        """
        # TODO: WTF?!
        num_repeat = x.shape[2] // xtalpha.shape[2]
        output_binary_start = x * xtalpha.repeat([1, 1, num_repeat]) - output_decimal.repeat([1, 1, num_repeat]) * epsalpha.repeat([1, 1, num_repeat])
        output_binary = dec2bin((output_binary_start / 2 + 0.5) * 256, 8)
        ...
        return self.binary_mlp(
            output_binary, 
            self_mask=conditions['self_mask'], 
            door_mask=conditions['door_mask'], 
            gen_mask=conditions['gen_mask']
        )
        
    def forward(self, x, timesteps, xtalpha, epsalpha, **conditions):
        """
        ...
        Args:
            ...
            xtalpha (torch.Tensor): TODO: Add description. Shape: (batch_size, sequence_length, 9).
            epsalpha (torch.Tensor): TODO: Add description. Shape: (batch_size, sequence_length, 9).
            **conditions: Conditions for the model.
        ...
        x = self.expand_points(x, conditions['connections'])
        ...
        condition_embeddings = self.condition_embeddings(
            torch.cat([conditions[key] for key in self.condition_keys], 2).float()
        )

        embeddings = input_embeddings + time_embeddings + condition_embeddings

        for layer in self.transformer_layers:
            embeddings = layer(embeddings, conditions['door_mask'], conditions['self_mask'], conditions['gen_mask'])

        output_decimal = self.analog_mlp(embeddings)
        output_binary = self.compute_binary_output(x, output_decimal, xtalpha, epsalpha, conditions, condition_embeddings)
