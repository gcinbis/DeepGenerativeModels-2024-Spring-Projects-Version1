...
def raster_to_json(line, print_door_warning):
    ...
    if os.path.exists(os.path.join(os.getcwd(),"rplan_json",f"{fp_id}.json")):
        print("This floorplan is already procesessed skipping...")
        return
    ...
    ### saving json files
    os.makedirs(os.path.join(os.getcwd(),"rplan_json"), exist_ok=True)
    with open(os.path.join(os.getcwd(),"rplan_json",f"{fp_id}.json") ,"w") as f:
         json.dump(info, f)
