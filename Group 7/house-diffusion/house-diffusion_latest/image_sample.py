    ...
    for k in doors_inds:
        for l in rooms_inds:
            if k > l:
                ...
                iou = p1.intersection(p2).area/ (p1.union(p2).area+0.001)
...
def main():
    ...
    batch_size=16
    use_ddim=False
    model_path="./ckpts/new_train/model000010.pt"
    draw_graph=True
    save_svg=True
    ...
    for _ in range(5):
        ...
        while tmp_count < num_samples:
            ...
            graph_error = save_samples(sample_gt, 'gt', model_kwargs, tmp_count, num_room_types, ID_COLOR=ID_COLOR, draw_graph=True, save_svg=True)
            graph_error = save_samples(sample, 'pred', model_kwargs, tmp_count, num_room_types, ID_COLOR=ID_COLOR, draw_graph=True, save_svg=True)
